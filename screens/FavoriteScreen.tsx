...
const FavoriteScreen = ({ navigation }: any) => {
  ...
  const [favList, setFavList] = useState<FavoriteProductTypes[]>([]);
  const [realProduct, setRealProduct] = useState<ProductTypes[]>([]);
  ...
  // useEffect(() => {
  //   fetch(`http://localhost:3000/favoriteItems/1`)
  //     .then((response) => response.json())
  //     .then((json) => {
  //       setFavList(json);
  //       const waitArr: any = [];
  //       favList.map((item: FavoriteProductTypes) => {
  //         product.map((data: ProductTypes) => {
  //           if (item.productId === data.id) {
  //             waitArr.push(data);
  //           }
  //         });
  //       });
  //       setRealProduct(waitArr);
  //       console.log(waitArr);
  //     })
  //     .catch((error) => {
  //       console.error(error);
